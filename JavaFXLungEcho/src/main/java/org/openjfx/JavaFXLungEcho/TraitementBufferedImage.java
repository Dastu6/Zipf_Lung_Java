...
public class TraitementBufferedImage {
	...
	public void BufferedImageToSonogram() {
		...
		float prevHGX = midWidth; prevHGY = h0;
		...
		float prevBGX = gOmega; prevBGY = newHeight;
		...
		booleanZipfMatrix = new boolean[newHeight][newWidth]; //Matrice qui va sauvegarder si le point doit être traité par Zipf
		...
		for(int i=0;i<nbThreadTraitement;i++)
		{
			 ThreadSonoTraitementImage temp = new ThreadSonoTraitementImage
					 (nbThreadTraitement, i, echographyImg,greyMatrixOnlySonogram,greyPixelsLevels,
							 pointsPenteGauche,pointsPenteDroite,pointsCourbeHaute,pointsCourbeBasseGauche,pointsCourbeBasseDroite,h0,gOmega,dOmega,h2);
			 temp.run();
		}
